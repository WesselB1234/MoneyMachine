...
=======
>>>>>>> parent of 8cc3a35 (Merge pull request #48 from Chris-17Lommerse/development)
...
public class GlobalExceptionHandler {
    ...
    private String getLocationOfException(Exception ex) {
        ...
        System.err.println(
                String.format("[RUNTIME ERROR] Reason: %s | Code Location: %s", ex.getMessage(), locationInfo));

        return locationInfo;
    }

    private ErrorResponse generateErrorDtoByExceptionAndErrorInfo(Exception ex, int code, ErrorType errorType,
            String message) {
    ...
    public ResponseEntity<ErrorResponse> handleNotFoundException(NotFoundException ex) {
        ErrorResponse errorDTO = generateErrorDtoByExceptionAndErrorInfo(ex, 404, ErrorType.NOT_FOUND,
                "Object not found");
        return ResponseEntity.status(errorDTO.getCode()).body(errorDTO);
    }
